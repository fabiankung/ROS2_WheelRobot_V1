...
class SerialComNode(Node):  # Create a class, inherited from the Node class.
    ...
    def __init__(self):     # Constructor.
        ...
        self.publisher_ = self.create_publisher(RCStatus, "tpc_RC_Status", 8)
        ...
        self.subscriber_ = self.create_subscription(RCCommand, 
                                                    "tpc_RC_Command",self.Subscriber_Callback,8)
    ...
    1. Periodic RC status packet which report the hardware status of the robotic platform
       ...
       5      Velocity.1 - MSB, or bits 15-8 of 16 bits signed integer representing average 
                           velocity in (revolution/sec) x 100 + 1024.
       ...
       15     Hardware status flags - Reserved
       16     Hardware status flags - Reserved
       17     Hardware status flags - Reserved
       18     Hardware status flags - Reserved
       19     Checksum byte, using sum complement method.
            ...
            if bytetoread > 20:  # At least 20 bytes
                ...
                if (sum == 0) or (sum == 256): # Make sure checksum is correct.
                    if ReadData[0] == 0x62:    # Compare with character 'b'.
                        ...
                        # Get velocity. Convert the bytes into 16-bits integer. Note that there is an offset which we must
                        # subtract off.
                        msg.velocity = int.from_bytes([ReadData[5], ReadData[6]], byteorder='big',signed=True) - 1024
